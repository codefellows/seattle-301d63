...
function displayPics(){
  ...
  // 'rando' is being declared with 'const' inside the while loop. The console.log() is trying to call the 'rando' variable but it's outside of the scope that 'rando' was declared.
  console.log(viewed);
...
function showList() {
  for(let i = 0; i < allProducts.length; i++) {
    const jimny = document.createElement('li');
    jimny.textContent = `${allProducts[i].name} has ${allProducts[i].votes} votes in ${allProducts[i].views} views`;
    list.appendChild(jimny);
  }
}
